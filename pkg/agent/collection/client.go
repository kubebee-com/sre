...
var ErrRuntime = errors.New("collector operation unavailable")
var ErrReenrollment = errors.New("collector authorization rejected; administrator re-enrollment required")
...
func (c *controlClient) post(ctx context.Context, path, token string, in, out any) error {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, c.origin+path+"?"+q.Encode(), bytes.NewReader(b))
	if err != nil {
		return ErrRuntime
	}
...
func (c *controlClient) enroll(ctx context.Context, token, uid string) (fleet.Credential, error) {
	...
	err := c.post(ctx, "/agent/enroll", "", map[string]string{"token": token, "cluster_uid": uid, "role": fleet.Collector}, &out)
	return out, err
}
func (c *controlClient) renew(ctx context.Context, old fleet.Credential) (fleet.Credential, error) {
	var out fleet.Credential
	err := c.post(ctx, "/agent/renew", old.Token, map[string]string{"role": fleet.Collector}, &out)
	return out, err
}
func (c *controlClient) report(ctx context.Context, token string, report Report) error {
	if report.Validate() != nil {
		return ErrRuntime
	}
	return c.post(ctx, "/agent/reports", token, report, nil)
}
...
func saveCredential(path string, c fleet.Credential) error {
